...
class ShoppingCart extends Query{
  ...
  public function addItem( $product_id, $quantity ){
    ...
    try{
      ...
      if( $adding['success'] == false && $adding['errors']['error_no'] == '1062' ){
        ...
        $item_query = " SELECT product_id,quantity 
  ...
  public function removeItem( $cart_id, $product_id ){
    if( $this -> account_id ){
      $remove_query = "DELETE FROM shopping_cart_item 
                        WHERE cart_id = UNHEX(?)
                        AND product_id = ?";
  ...
  public function getCartItemsSkis( $cart_id ){
    ...
    if( $this -> account_id ){
      ...
      $get_items_query = "SELECT @product_id := shopping_cart_item.product_id AS product_id,
                          ...
                          ON shopping_cart_item.product_id = product_ski.product_id
                          INNER JOIN product_quantity
                          ON product_quantity.product_id = product_ski.product_id
                          WHERE cart_id = UNHEX( ? );";
      ...
      if( isset( $_SESSION['cart']['items']) && is_array( $_SESSION['cart']['items'] ) ){
        ...
        $query = "SELECT @product_id := product_ski.product_id AS product_id,
                  product_ski.name,
                  product_ski.description,
                  product_ski.price,
                  product_quantity.quantity as available,
  ...
  public function updateCartItem( $cart_id, $product_id, $quantity ){
    ...
    if( $this -> account_id ){
      $update_query = "UPDATE shopping_cart_item 
                        SET quantity = ?
                        WHERE cart_id = UNHEX( ? )
                        AND product_id = ?";
  ...
  private function addNewCartItem( $cart_id, $product_id, $quantity ){
    $response = array();
    if( $this -> account_id ){
      $add_query = "INSERT INTO shopping_cart_item(cart_id, product_id, quantity)
                    VALUES( UNHEX( ? ), ?, ? )";
  ...
  public function getCartId( $create = true ){
    ...
    if( $this -> account_id ){
      $find_cart_query = "SELECT HEX( cart_id ) as cart_id 
                          FROM shopping_cart 
                          WHERE active = 1 
  ...
  private function createCart(){
    ...
    if( $this -> account_id ){
      $new_cart_query = " INSERT INTO shopping_cart (cart_id,account_id)
                          VALUES ( UNHEX(?) , UNHEX(?) )";
